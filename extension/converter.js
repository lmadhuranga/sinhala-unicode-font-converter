/*
  Sinhala Singlish -> Unicode converter
  Adapted from gisi.js in this repository.
*/
...
(function () {
  ...
  const specialConsonantsUni = [];
  const specialChar = [];
  const specialCharUni = [];
  const consonants = [];
  const consonantsUni = [];
  ...
  function convertSinglishToUnicode(input) {
    let text = input || "";
    ...
    for (let i = 0; i < specialCharUni.length; i++) {
      for (let j = 0; j < consonants.length; j++) {
        const a = consonants[j] + specialChar[i];
        const c = consonantsUni[j] + specialCharUni[i];
    ...
    for (let j = 0; j < consonants.length; j++) {
      for (let i = 0; i < vowels.length; i++) {
        const a = consonants[j] + "r" + vowels[i];
        const c = consonantsUni[j] + "්‍ර" + vowelModifiersUni[i];
        const b = new RegExp(a, "g");
        text = text.replace(b, c);
      }
      const a = consonants[j] + "r";
      const c = consonantsUni[j] + "්‍ර";
      const b = new RegExp(a, "g");
      text = text.replace(b, c);
    }

    for (let j = 0; j < consonants.length; j++) {
      for (let i = 0; i < nVowels; i++) {
        const a = consonants[j] + vowels[i];
        const c = consonantsUni[j] + vowelModifiersUni[i];
        const b = new RegExp(a, "g");
        text = text.replace(b, c);
      }
    }
    ...
    for (let j = 0; j < consonants.length; j++) {
      const b = new RegExp(consonants[j], "g");
      text = text.replace(b, consonantsUni[j] + "්");
    }

    for (let i = 0; i < vowels.length; i++) {
      const b = new RegExp(vowels[i], "g");
      text = text.replace(b, vowelsUni[i]);
    }

    return text;
  }

  window.__sinhalaConverter = { convertSinglishToUnicode };
})();
